````markdown
# Grid Decomposition for Continuous Space MDP

This conversation covers the implementation of grid decomposition for solving a continuous space Markov Decision Process (MDP) using value iteration with analytical transition probabilities.

---

## Initial Problem Description

### Week 1: Pure Particle Dynamics

- Continuous state space: $x(t) = \begin{pmatrix} y(t) \\ v(t) \end{pmatrix} \in \mathbb{R}^2$
- Dynamics with Gaussian noise, speed wobble, crash probability, and sensor noise
- Force field: $\phi(y)$
- System parameters: mass $m$, max velocity $v_{max}$, crash probability $p_c$

### Week 2: Discrete Simplification

- State space becomes integer: $s[t] = \begin{pmatrix} y[t] \\ v[t] \end{pmatrix} \in \mathbb{Z}^2$
- Discrete actions: $a[t] \in \{-1, 0, 1\}$
- Force field: $f_\phi(y) = \lfloor A\sin(2\pi y/y_{max}) \rfloor$
- Speed wobble $d(v) \in \{-1, 0, 1\}$ with velocity-dependent probabilities
- Goal: Reach $(0, 0)$ to receive reward of 1

### Week 3: Back to Continuous Space

- Need to discretize continuous space using grid decomposition
- Train policy on discretized "mock" system
- Test policy on original continuous system

---

## Grid Decomposition Mathematics

### Continuous State Space

$$\mathcal{S} = \{(y, v) : y \in [-y_{max}, y_{max}], v \in [-v_{max}, v_{max}]\} \subset \mathbb{R}^2$$

### Uniform Grid Partition

Define grid resolution parameters $\Delta_y, \Delta_v > 0$:

$$\mathcal{G}_y = \{y_i = -y_{max} + i\Delta_y : i = 0, 1, \ldots, N_y\}$$
$$\mathcal{G}_v = \{v_j = -v_{max} + j\Delta_v : j = 0, 1, \ldots, N_v\}$$

where:
$$N_y = \left\lfloor \frac{2y_{max}}{\Delta_y} \right\rfloor, \quad N_v = \left\lfloor \frac{2v_{max}}{\Delta_v} \right\rfloor$$

**Discrete State Space:**
$$\mathcal{S}_d = \mathcal{G}_y \times \mathcal{G}_v$$

**Cardinality:**
$$|\mathcal{S}_d| = (N_y + 1)(N_v + 1) = O\left(\frac{y_{max}}{\Delta_y} \cdot \frac{v_{max}}{\Delta_v}\right)$$

### State Quantization Function

**Quantization Map** $Q: \mathbb{R}^2 \to \mathcal{S}_d$:

$$Q(y, v) = (y_i, v_j)$$

where:
$$i = \left\lfloor \frac{y + y_{max}}{\Delta_y} \right\rfloor, \quad j = \left\lfloor \frac{v + v_{max}}{\Delta_v} \right\rfloor$$

### Discretized Bellman Equation

**Continuous Bellman Optimality Equation:**
$$V^*(s) = \max_{a \in \mathcal{A}} \int_{\mathcal{S}} p(s'|s,a)[R(s,a,s') + \gamma V^*(s')] \, ds'$$

**Discretized Version:**
$$V^*_d(s_d) = \max_{a \in \mathcal{A}} \sum_{s'_d \in \mathcal{S}_d} \hat{P}(s'_d | s_d, a)[R(s_d, a, s'_d) + \gamma V^*_d(s'_d)]$$

**Value Iteration:**
$$V_{k+1}(y_i, v_j) = \max_{a \in \{-1, 0, 1\}} \sum_{\ell=0}^{N_y} \sum_{m=0}^{N_v} \hat{P}((y_\ell, v_m) | (y_i, v_j), a) \cdot [R_{ij\ell m}^a + \gamma V_k(y_\ell, v_m)]$$

---

## Why Analytical vs Monte Carlo?

### Analytical Approach (Used Here)

For dynamics with known probability distributions:

- **Advantages:**
  - 10-30x faster than Monte Carlo
  - Exact probabilities (no sampling error)
  - Deterministic results
  - No need to tune number of samples
- **When to use:** Simple noise distributions (Gaussian, discrete, etc.)

### Monte Carlo Approach

- **When needed:**
  - Complex nonlinear noise
  - Unknown/black-box dynamics
  - High-dimensional complex systems

### Transition Probability Computation

For your dynamics with Gaussian noise $\mathcal{N}(0, 0.01v^2)$:

$$v' = v + \frac{1}{m}f_{net} + d(v) + \mathcal{N}(0, 0.01v^2)$$

The transition involves:

1. **Deterministic component:** $\bar{v}' = v + \frac{1}{m}f_{net}$
2. **Discrete wobble:** $d(v) \in \{-1, 0, 1\}$ with known probabilities
3. **Continuous noise:** $\epsilon \sim \mathcal{N}(0, 0.01v^2)$
4. **Crash probability:** Binary event with probability $p_c \cdot |v|/v_{max}$
...
class ContinuousParticleSystem:
    ...
    def __init__(self, m, y_max, v_max, A, p_c, p_w, dt=0.1):
        self.m = m
        self.y_max = y_max
        self.v_max = v_max
        self.A = A
...
### Step 2: Grid System with Analytical Transitions
...
class GridParticleSystem:
    ...
    def __init__(self, continuous_system, delta_y, delta_v):
        ...
        self.y_grid = np.arange(
            -continuous_system.y_max,
            continuous_system.y_max + delta_y,
            delta_y
        )
        self.v_grid = np.arange(
            -continuous_system.v_max,
            continuous_system.v_max + delta_v,
            delta_v
        )
    ...
    def discretize(self, y, v):
        ...
        i = int(np.clip(
            (y + self.continuous.y_max) / self.delta_y,
            0, self.n_y - 1
        ))
        j = int(np.clip(
            (v + self.continuous.v_max) / self.delta_v,
            0, self.n_v - 1
        ))
        return i, j
            ...
            for wobble_val, p_wobble in wobble_dist.items():
                ...
                for j_next in range(self.n_v):
                    ...
                    if noise_std < 1e-6:
                        ...
                        p_v_in_cell = norm.cdf(v_cell_high_clipped, v_mean, noise_std) - \
...
class PolicyEvaluator:
    ...
    def evaluate(self, n_episodes=100, verbose=True):
        ...
        results['avg_steps_success'] = np.mean([
            s for s, r in zip(results['steps_list'], results['rewards_list']) if r > 0
        ]) if results['successes'] > 0 else float('inf')
...
def visualize_policy(vi, grid_sys, save_path='policy_visualization.png'):
    ...
    im1 = ax1.imshow(
        ...
        aspect='auto',
        cmap='viridis',
        extent=[-grid_sys.continuous.y_max, grid_sys.continuous.y_max,
                -grid_sys.continuous.v_max, grid_sys.continuous.v_max]
    )
    ...
    scatter = ax2.scatter(Y_pos, V_pos, c=colors, cmap='RdYlGn',
                         s=50, vmin=-1, vmax=1, alpha=0.7)
...
1. **Grid Decomposition**: Discretize continuous space into finite cells
2. **Analytical Transitions**: Use known probability distributions instead of sampling
3. **Value Iteration**: Solve Bellman equation iteratively on discrete grid
4. **Policy Transfer**: Map continuous states to grid for action lookup
5. **Evaluation**: Test learned policy on original continuous system
...
```
[Continuous System] ──discretize──> [Grid System] ──solve──> [Policy] ──test──> [Continuous System]
   (real world)                      (tractable)              (discrete)         (evaluate)
```

1. Create continuous system with stochastic dynamics
2. Discretize into uniform grid
...
- **Space:** $O(N_y \cdot N_v)$
- **Time per iteration:** $O(N_y \cdot N_v \cdot |\mathcal{A}| \cdot K)$ where $K$ is avg. non-zero transitions
- **Convergence:** $k = O\left(\frac{\log(1/\epsilon)}{\log(1/\gamma)}\right)$ iterations

---

## Resolution Tradeoffs

| Resolution | Grid Size     | Computation | Accuracy |
| ---------- | ------------- | ----------- | -------- |
| Δ = 1.0    | 20×10 = 200   | Fast        | Low      |
| Δ = 0.5    | 40×20 = 800   | Medium      | Medium   |
| Δ = 0.25   | 80×40 = 3,200 | Slow        | High     |

Finer grids provide better approximations but require more computation.
